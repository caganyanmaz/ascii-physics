...
void SpringGenerator::generate(std::vector<Particle>& particles) {  
    ...
    Vec2 I        = particles[a_id].position - particles[b_id].position;
    Vec2 diff_I   = particles[a_id].velocity - particles[b_id].velocity;
    ...
    Vec2 f_a = -(spring_factor + damping_factor) * I / norm_I;
    Vec2 f_b = (-1) * f_a;
